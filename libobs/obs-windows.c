...
#endif
static void get_base_path(struct dstr *dir_path){
    wchar_t tmp_path[MAX_PATH] = { 0 };
    wchar_t *p = NULL;
    GetModuleFileNameW((HMODULE)&__ImageBase, tmp_path, MAX_PATH - 1);
    p = wcsrchr(tmp_path, L'\\');
    if (NULL != p)
        *p = 0;
    dstr_ensure_capacity(dir_path, MAX_PATH + 1);
    dir_path->len = os_wcs_to_utf8(tmp_path, wcslen(tmp_path), dir_path->array,
        dir_path->capacity);
...
static bool check_data_path(const char *file, const char *path,
    struct dstr *output)
{
    struct dstr base_dir_path;
    dstr_init(&base_dir_path);
    get_base_path(&base_dir_path);
    dstr_copy_dstr(output, &base_dir_path);
    dstr_cat(output, path);
    dstr_cat(output, file);
    dstr_free(&base_dir_path);
...
char *find_libobs_data_file(const char *file)
{
    ...
    if (check_data_path(file, "data/libobs/", &path))
        return path.array;

    if (check_data_path(file, "obs/data/libobs/", &path))
        return path.array;
